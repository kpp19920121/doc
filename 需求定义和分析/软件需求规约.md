...
|实体类名称|属性|关联类|定义|
|:-:|:-:|:-:|:-:|
| User | UserID: int<br />Username: String<br />Password: String<br />E-mail: String<br />UserProfile: Imageformat |  | 用户信息类，保存用户名、密码、邮箱及用户ID |
| SoundSource | SoundSourceID: int<br />SoundFragment: Soundformat<br />SoundName: String<br />SoundLatitude: double<br />SoundLongitude: double<br />SoundAltitude: double<br />SoundVolume: int<br />SoundPattern: String | | 虚拟声源类，保存虚拟声源文件、虚拟声源名称、经纬度、高度、音量、播放模式及虚拟声源ID |
| VirtualSoundSpace | SoundSources: List<br />VSSName: String<br />VSS_ID: int<br />OwnerID: int<br />VSS_Description: String | | 保存该用户上传的所有虚拟声源的列表，保存虚拟声源空间的名称、所有者ID、虚拟声源空间ID |
| Comment | CommenterID: int<br />Comments: String | | 保存评论者ID以及其评论 |
| CommentList | Comment: List | | 保存VSS的评论列表 |
| Like | LikerIDs: List<br />LikeCount: int | | 保存点赞用户列表以及点赞数目 |
| SoundSpaceLibrary | VirtualSoundSpaces: List<br />LibraryID: int<br />LibraryName: String<br />Comments: List<br />Likes: List | | 保存用户制作的所有VSS的列表、库ID、库名称、用户评论及用户点赞列表 |
| Sensor | Status: String<br />SensorID: int | | 保存与软件适配的传感器状态及传感器ID |
##### 表 边界类定义表
|边界类名称|定义|
|:-:|:-:|
| RegisterPage | 注册页面 |
...
| VSSLibraryControl | 控制用户对本地VSS库的添加、删除、上传、下载的过程 |
