...
|2|用户在本地音频库中不断选择音频上传，并设置其经纬度、高度等位置信息和音量、循环模式等播放信息|
...
|4|用户选择上传的VSS已经在本地库中消失，成为在线VSS库中的已缓存VSS|
|| **扩展流程** |
||用户输入名字不合法|
|1|在第2步时，如果与现有VSS重名，则提示重名并要求用户重新输入|
|2|在第2步时，如果输入名字带有非法字符，则提示用户重新输入|
...
|2|如果是本地VSS，用户可以进入用例PlayVSS；如果是在线VSS，用户可以进入用例DownloadVSS，LikeVSS和CommentVSS|
...
|| **主流程** |
|1|用户浏览在线库，界面上出现所有用户上传的VSS|
|2|用户选择某个VSS进行预览，进入用例PreviewVSS|
|3|用户完成浏览|
|| **扩展流程** |
||无|
| **特殊需求** |无|
...
|| **主流程** |
|1|系统提示是否确认下载该在线VSS|
|2|用户确认下载，之后系统将在线VSS下载到本地，并保存在本地VSS中|
...
|| **主流程** |
|1|系统提示用户进行传感器校准，进入用例AdjustSensor，之后用户正确佩戴传感器|
...
| **后置条件** | 无 |
|| **主流程** |
|1|系统读取该VSS的音频信息，并读取用户位置，进入游览界面，提示用户佩戴传感器和耳机|
...
| **后置条件** | 无 |
|| **主流程** |
|1|系统读取该VSS的音频信息，进入游览界面，提示用户佩戴传感器和耳机|
...
|实体类名称|属性|关联类|定义|
|:-:|:-:|:-:|:-:|
| User | userID: int<br />username: String<br />password: String<br />e-mail: String<br />userProfile: Imageformat | VirtualSoundSpace<br />SoundSource<br />Sensor<br />SoundSpaceLibrary | User类保存了使用该系统的一个用户的相关信息，包括用户名、密码、邮箱及用户ID |
| SoundSource | soundSourceID: int<br />soundFragment<br />soundName: String<br />soundLatitude: double<br />soundLongitude: double<br />soundAltitude: double<br />soundVolume<br />soundPattern: String | VirtualSoundSpace<br />User | SoundSource类是用户为创建VSS，从本地VSS库可以访问到的所有VSS中提取出来的SoundSource或本地直接上传到系统上的虚拟声源，保存音频片段文件、虚拟声源名称、经纬度、高度、音量、播放模式及虚拟声源ID |
| MapModeVirtualSoundSpace | soundSources: List<br />VSS_Name: String<br />VSS_ID: int<br />ownerID: int<br />VSS_Description: String<br />status: String | User<br />SoundSource<br />SoundSpaceLibrary | MapModeVirtualSoundSpace 类是用户创建的Map Mode下的VSS实例，虚拟音频的形成将与用户绝对地理位置有关，存储在SoundSpaceLibrary中，保存单个VSS中用户上传的所有虚拟声源的列表，保存虚拟声源空间的名称、所有者ID、虚拟声源空间ID以及VSS状态 |
| ConcertModeVirtualSoundSpace | soundSources: List<br />VSS_Name: String<br />VSS_ID: int<br />ownerID: int<br />VSS_Description: String<br />status: String | User<br />SoundSource<br />SoundSpaceLibrary | ConcertModeVirtualSoundSpace类是用户创建的Concert Mode下的VSS实例，虚拟音频的形成与用户绝对地理位置无关，存储在SoundSpaceLibrary中，保存单个VSS中用户上传的所有虚拟声源的列表，保存虚拟声源空间的名称、所有者ID、虚拟声源空间ID以及VSS状态 |
| LocalSoundSpaceLibrary | virtualSoundSpaces: List<br />libraryID: int<br />libraryName: String | MapModeVirtualSoundSpace<br />ConcertModeVirutalSoundSpace<br />User | LocalSoundSpaceLibrary类是用户的所有VSS的本地资源库，包括已经下载完毕的和用户自己制作的MapModeVSS以及ConcertModeVSS，保存用户制作的所有VSS的列表、库ID、库名称 |
| OnlineSoundSpaceLibrary | virtualSoundSpaces: List<br />libraryID: int<br />libraryName: String | MapModeVirtualSoundSpace<br />ConcertModeVirutalSoundSpace<br />User | OnlineSoundSpaceLibrary类是供用户浏览的其他用户所制作上传的在线VSS资源库，包括MapModeVSS以及ConcertModeVSS，保存这些VSS的列表、库ID、库名称 |
| Comment | commenterID: int<br />comment: String | － | 保存评论者ID以及其评论 |
| Like | likerIDs: List<br />likeCount: int | － | 保存点赞用户列表以及点赞数目 |
...
| PersonalInfoPage | User信息表单，以说明在登录或进行相关VSS操作时的User信息 |
...
| VSSPreviewerPage | 系统收到预览VSS的消息后展示的VSS预览界面，以向用户展示他希望预览的VSS，并提供社交活动界面包括评论和点赞 |
...
| VSSControl | 控制用户导入音频生成VSS的过程 |
| LocalVSSLibraryControl | 控制用户制作新的VSS、删除VSS、重命名VSS、上传VSS、预览及游览本地VSS的过程 |
| OnlineVSSLibraryControl | 控制用户预览及游览在线VSS、下载VSS的过程 |
| SensorControl | 控制用户调整传感器的过程 |


所有实体对象类类图如下所示

![实体对象类类图](https://github.com/AudioAR/doc/blob/master/%E9%9C%80%E6%B1%82%E5%AE%9A%E4%B9%89%E5%92%8C%E5%88%86%E6%9E%90/ClassDiagram/%E5%AE%9E%E4%BD%93%E5%AF%B9%E8%B1%A1%E7%B1%BB%E5%9B%BE.svg)

所有类的类图如下所示

![所有对象类图](https://github.com/AudioAR/doc/blob/master/%E9%9C%80%E6%B1%82%E5%AE%9A%E4%B9%89%E5%92%8C%E5%88%86%E6%9E%90/ClassDiagram/%E6%89%80%E6%9C%89%E7%B1%BB%E5%9B%BE.svg)

#### 3.4.4动态模型
##### 1）系统顺序图与操作契约

详见[动态模型_系统顺序图和操作契约](./系统顺序图与操作契约/动态模型_系统顺序图和操作契约.md)

##### 2）顺序图
###### 1.Register
![SeqDiagram](./SeqDiagram/register.svg)
###### 2.Login
![SeqDiagram](./SeqDiagram/login.svg)
###### 3.ManageLocalVSSLibrary
![SeqDiagram](./SeqDiagram/managelocalvsslibrary.svg)
###### 4.CreateVSS
![SeqDiagram](./SeqDiagram/createvss.svg)
###### 5.CreateMapModeVSS
![SeqDiagram](./SeqDiagram/createmapmodevss.svg)
###### 6.CreateConcertModeVSS
![SeqDiagram](./SeqDiagram/createconcertmodevss.svg)
###### 7.RenameVSS
![SeqDiagram](./SeqDiagram/renamevss.svg)
###### 8.DeleteVSS
![SeqDiagram](./SeqDiagram/deletevss.svg)
###### 9.UploadVSS
![SeqDiagram](./SeqDiagram/uploadvss.svg)
###### 10.PreviewVSS
![SeqDiagram](./SeqDiagram/previewvss.svg)
###### 11.BrowseOnlineVSSLibrary
![SeqDiagram](./SeqDiagram/browseonlinevsslibrary.svg)
###### 12.DownloadVSS
![SeqDiagram](./SeqDiagram/downloadvss.svg)
###### 13.LikeVSS
![SeqDiagram](./SeqDiagram/likevss.svg)
###### 14.CommentVSS
![SeqDiagram](./SeqDiagram/commentvss.svg)
###### 15.PlayVSS
![SeqDiagram](./SeqDiagram/playvss.svg)
###### 16.PlayMapModeVSS
![SeqDiagram](./SeqDiagram/playmapmodevss.svg)
###### 17.PlayConcertModeVSS
![SeqDiagram](./SeqDiagram/playconcertmodevss.svg)
###### 18.AdjustSensor
![SeqDiagram](./SeqDiagram/adjustsensor.svg)
##### 3）状态图
...
**This table need to be updated, or removed in the future.**
...
|VSS详情界面|Preview|BrowseOnlineVSS \ ManageLocalVSS| ManageLocalVSS \ BrowseOnlineVSS \ AdjustSensor|查看VSS详情，根据不同VSS类型有不同的Preview信息产生|
|在线VSS浏览界面|BrowseOnlineVSS|Config \ ManageLocalVSS|Config \ ManageLocalVSS \ Preview|浏览在线分享的VSS|
|配置界面|Config| BrowseOnlineVSS \ ManageLocalVSS | BrowseOnlineVSS \ ManageLocalVSS | 软件相关的配置选项，可以从任何入口进入|
|音乐厅模式VSS体验|PlayConcertModeVSS|AdjustSensor|Preview|-|
|地图模式VSS体验|PlayMapModeVSS|Preview \ ManageLocalVSS| Preview \ ManageLocalVSS|-|
|传感器校准|AdjustSensor|AdjustSensor| PlayConcertModeVSS \ PlayMapModeVSS| -|
...
** Preview 根据内容的类型进行判断，显示不同的内容，这里通过下划线进行不同显示内容的区分。 **

** AdjustSensor 根据内容的不同进行判断，进行不同的校准步骤，这里通过下划线进行不同界面的区分。 **
