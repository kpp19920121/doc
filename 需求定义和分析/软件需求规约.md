...
|PreviewVSS|子功能|预览虚拟声音空间，查看空间信息与其中音频片段的详细信息|
...
|          实体类名称          |                             属性                             |                            关联类                            |                             定义                             |
| :-------------------------- | :---------------------------------------------------------- | :---------------------------------------------------------- | :---------------------------------------------------------- |
|             User             | userID: int<br />username: String<br />password: String<br />e-mail: String |             LocalSoundLibrary<br />Sensor<br />              | User类保存了使用该系统的一个用户的相关信息，包括用户名、密码、邮箱及用户ID。简单期间 |
|         SoundSource          | soundSourceID: int<br />soundFragment<br />soundName: String<br />soundLatitude: double<br />soundLongitude: double<br />soundAltitude: double<br />soundVolume<br />soundPattern: String |  MapModeVirtualSoundSpace<br />ConcertModeVirtualSoundSpace  | SoundSource类是用户为创建VSS，从本地VSS库可以访问到的所有VSS中提取出来的SoundSource或本地直接上传到系统上的虚拟声源，保存音频片段文件、虚拟声源名称、经纬度、高度、音量、播放模式及虚拟声源ID |
|VirtualSoundSpace|Comments: List<Comment\><br />Likes: List<Like\><br/>soundSources: List<SoundSource\><br />VSS_Name: String<br />VSS_ID: int<br />ownerID: int<br />VSS_Description: String<br />status: String||两种不同的SoundSpace的抽象类，包含了两种不同的VSS的公共信息。其中关于评论和喜爱的信息只适用于在线VSS，为了不使模型过于复杂，不进行在线类和本地类的区分，而仅仅使用一个状态属性|
|   MapModeVirtualSoundSpace   | soundSources: List<SoundSource\><br />VSS_Name: String<br />VSS_ID: int<br />ownerID: int<br />VSS_Description: String<br />status: String | LocalSoundSpaceLibrary<br />OnlineSoundSpaceLibrary<br />SoundSource | MapModeVirtualSoundSpace 类是用户创建的Map Mode下的VSS实例，虚拟音频的形成将与用户绝对地理位置有关，存储在SoundSpaceLibrary中，保存单个VSS中用户上传的所有虚拟声源的列表，保存虚拟声源空间的名称、所有者ID、虚拟声源空间ID以及VSS状态。 |
| ConcertModeVirtualSoundSpace | soundSources: List<SoundSource\><br />VSS_Name: String<br />VSS_ID: int<br />ownerID: int<br />VSS_Description: String<br />status: String | LocalSoundSpaceLibrary<br />OnlineSoundSpaceLibrary<br />SoundSource | ConcertModeVirtualSoundSpace类是用户创建的Concert Mode下的VSS实例，虚拟音频的形成与用户绝对地理位置无关，存储在SoundSpaceLibrary中，保存单个VSS中用户上传的所有虚拟声源的列表，保存虚拟声源空间的名称、所有者ID、虚拟声源空间ID以及VSS状态 |
|    LocalSoundSpaceLibrary    | virtualSoundSpaces: List\<VirtualSoundSpace\><br />| User<br />MapModeVirtualSoundSpace<br />ConcertModeVirtualSoundSpace | LocalSoundSpaceLibrary类是用户的所有VSS的本地资源库，包括已经下载完毕的和用户自己制作的MapModeVSS以及ConcertModeVSS，保存用户制作的所有VSS的列表 |
|   OnlineSoundSpaceLibrary    | virtualSoundSpaces: List<VirtualSoundSpace\><br /> | User<br />MapModeVirtualSoundSpace<br />ConcertModeVirtualSoundSpace | OnlineSoundSpaceLibrary类是供用户浏览的其他用户所制作上传的在线VSS资源库，包括MapModeVSS以及ConcertModeVSS，保存这些VSS的列表、库ID、库名称 |
|           Comment            |            commenterID: int<br />comment: String             |                   OnlineSoundSpaceLibrary                    | Comment类保存预览VSS时用户对VSS评论的信息，包括评论者ID以及其评论 |
|             Like             |              likerIDs: List<br />likeCount: int              |                   OnlineSoundSpaceLibrary                    | Like类保存用户在预览VSS时对VSS点赞的信息，包括点赞者列表以及点赞个数 |
|            Sensor            |              status: String<br />sensorID: int               |                             User                             | Sensor类帮助用户调整传感器状态，保存与软件适配的传感器状态及传感器ID |
|            SoundFragment            |              downloaded: boolean<br>              |                             SoundSource                             | SoundFragment类作为音频文件的包装类，封装音频文件的使用方法，该类只被SoundSource单向引用。代表了一个已经下载的或者没有下载的音频文件。 |
##### 边界类定义表
|边界类名称|定义|
|:-|:-|
| RegisterPage | 新的User进入系统后的注册页面 |
| LoginPage | User进入系统时的登陆页面 |
| PersonalInfoPage | User信息表单，用于在注册和登录的时候进行填写。 |
| LocalVSSManagerPage | LocalSoundSpaceLibrary的管理界面，以向用户展示本地可用的VSS操作信息及方法 |
| OnlineVSSViewerPage | OnlineSoundSpaceLibrary的浏览界面，以向用户展示在线浏览其他用户VSS时可用的操作信息及方法 |
| PreviewOnlineMap | 用于在线的地图模式VSS预览，提供下载，评论和点赞功能 |
| PreviewLocalMap | 用于本地的地图模式VSS预览，提供上传和游览功能 |
| PreviewOnlineConcert | 用于在线的音乐会模式VSS预览，提供下载，评论和点赞功能 |
| PreviewLocalConcert | 用于本地的音乐会模式VSS预览，提供上传和游览功能 |
| VSSViewingPage | 系统收到用户对预览的VSS想要游览的消息后展示的VSS浏览界面，以向用户展示他希望游览的VSS界面 |
| CreateVSSPage | 用户在本地创建VSS的界面 |
| SensorPage | 传感器调整界面 |
##### 控制类定义表
|控制类名称|定义|
|:-:|:-:|
| UserInfoControl | 控制用户注册、登陆和查看个人本地VSS的过程 |
| VSSCreateControl | 控制用户导入音频生成VSS的过程 |
| LocalVSSLibraryControl | 控制用户制作新的VSS、删除VSS、重命名VSS、上传VSS、预览本地VSS的过程 |
| OnlineVSSLibraryControl | 控制用户预览及浏览在线VSS、下载VSS的过程 |
| SensorControl | 控制用户调整传感器的过程 |
| VSSPlayControl | 控制VSS回放的过程，负责一切各种VSS的回放工作 |

所有实体对象类类图如下所示

![实体对象类图](./ClassDiagram/实体对象类图.svg)
...
class SoundSource {
  -soundSourceID : int
  -soundLatitude : double
  -soundLongitude : double
  -soundAltitude : double
  -soundVolume
  -soundPattern : String
  -soundName : String
}
class MapModeVirtualSoundSpace {
  -soundSources : List
  -VSS_ID : int
  -ownerID : int
  -VSS_Name : String
  -VSS_Description : String
  -status : String
}
class ConcertModeVirtualSoundSpace {
  -soundSources : List
  -VSS_ID : int
  -ownerID : int
  -VSS_Name : String
...
class LocalSoundSpaceLibrary {
  -virtualSoundSpaces : List
  -libraryID : int
  -libraryName : String
}
...
User "1" -- "1" Sensor
User "1" -- "1" LocalSoundSpaceLibrary
User "1" -- "*" OnlineSoundSpaceLibrary

MapModeVirtualSoundSpace "1" o-- "1..*" SoundSource
ConcertModeVirtualSoundSpace "1" o-- "1..*" SoundSource

LocalSoundSpaceLibrary "1" o-- "*" MapModeVirtualSoundSpace
LocalSoundSpaceLibrary "1" o-- "*" ConcertModeVirtualSoundSpace

OnlineSoundSpaceLibrary "1" o-- "*" MapModeVirtualSoundSpace
OnlineSoundSpaceLibrary "1" o-- "*" ConcertModeVirtualSoundSpace

VirtualSoundSpace "1" o-- "*" Comment
VirtualSoundSpace "1" o-- "*" Like
...
![实体类对象继承关系](./ClassDiagram/实体对象继承关系.svg)
...
abstract SoundSpaceLibrary {
  -virtualSoundSpaces : List
  -libraryID : int
  -libraryName : String
}

abstract VirtualSoundSpace {
  -soundSources : List
  -VSS_ID : int
  -ownerID : int
  -VSS_Name : String
...
SoundSpaceLibrary <|-- LocalSoundSpaceLibrary
SoundSpaceLibrary <|-- OnlineSoundSpaceLibrary
...
![所有对象类图](./ClassDiagram/所有对象类图.svg)
...
class LocalVSSManagerPage
class OnlineVSSViewerPage
class VSSPreviewerPage
class VSSViewingPage
class CreateVSSPage
class SensorPage

class UserInfoControl
class VSSControl
class LocalVSSLibraryControl
class OnlineVSSLibraryControl
class SensorControl
...
VirtualSoundSpace --> Comment
VirtualSoundSpace --> Like

UserInfoControl "1" - "*" User
VSSControl "1" -- "*" MapModeVirtualSoundSpace
VSSControl "1" -- "*" ConcertModeVirtualSoundSpace
Sensor "*" - "1" SensorControl
OnlineVSSLibraryControl "1" -- "*" OnlineSoundSpaceLibrary
LocalVSSLibraryControl "1" -- "*" LocalSoundSpaceLibrary

UserInfoControl "1" -- "1" RegisterPage
UserInfoControl "1" -- "1" LoginPage
UserInfoControl "1" -left- "1" PersonalInfoPage

VSSControl "1" -- "1" CreateVSSPage

SensorControl "1" -- "1" SensorPage

LocalVSSLibraryControl "1" -- "1" VSSViewingPage
LocalVSSLibraryControl "1" -- "1"  VSSPreviewerPage
LocalVSSLibraryControl "1" -- "1" LocalVSSManagerPage

OnlineVSSLibraryControl "1" -- "1" OnlineVSSViewerPage
OnlineVSSLibraryControl "1" -- "1" VSSPreviewerPage
@enduml
```
...
[@./SeqDiagram.md](./SeqDiagram/SeqDiagram.md)
