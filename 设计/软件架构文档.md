...
###### 类图
![LocalVSSSubsystemClassDiagram](./Diagrams/LocalVSSSubsystemClassDiagram.svg)
@startuml Diagrams/LocalVSSSubsystemClassDiagram
...
这一子系统共提供六个功能接口，分别是上传VSS数据，上传SF，下载VSS数据，下载SF，登录，注册
...
|Signature|Description|
|:-|:-|
|int RegisterRequest(String username, String password, String email)|第一个返回值为询问结果，-1表示用户名已存在，0表示电子邮箱已被注册，1表示注册成功，第二个返回值为用户的ID，第一个返回值为-1或0的时候返回的ID为0|
...
| **范围** | 系统用例 |
| **前置条件** | App不在运行中 |
| **后置条件** | App完成初始化，用户进入LoginViewModel |
|| **主流程** |
|1|用户点击APP图标|
|2|系统创建LoginViewModel对象|
|3|LoginViewModel创建UserInfoControl对象|
|4|LoginViewModel创建GVRAudioEngine对象|
|5|LoginViewModel创建Callable对象|
|| **扩展流程** |
||无|
| **特殊需求** |无|
