...
具体服务器表单设计如下：
|Database|VSSSInformation||
|:-|:-|:-------------------:|:-|
|**Datatable**|**List of Fields**|**Description**|
|VSS_Table|CHAR(size) VSSID<br>CHAR(size) UserID<br>INT likeNum|(1)VSS的本地ID（为VSS的唯一标示）<br>(2)上传VSS的用户的ID<br>(3)点赞者的数量|
|VSS_SS_Table|CHAR(size) VSSID<br>CHAR(size) SFID<br>FLOAT lng<br>FLOAT lat<br>FLOAT height<br>INT volume<br>INT loop_mode|(1)SS所属的VSS的ID<br>(2)该SS中SF的唯一ID<br>(3)SS的经度<br>(4)SS的纬度<br>(5)SS的高度<br>(6)SS的音量<br>(7)SS的循环模式|
|VSS_SF_Table|CHAR(size) SFID<br>CHAR(size) filePath<br>CHAR(size) MD5_hash|(1)SF的ID<br>(2)SF（在服务器端）的文件位置<br>(3)SF的MD5哈希值（用于音频判重验证）|
|VSS_Like_Table|CHAR(size) VSSID<br>CHAR(size) LikeUserID|(1)包含VSS的ID<br>(2)点赞者的ID|
|VSS_Comment_Table|CHAR(size) VSSID<br>CHAR(size) CommentUserID<br>TIME CommentTime<br>TEXT CommentContent|(1)VSS的ID<br>(2)评论者的ID<br>(3)评论的时间<br>(4)评论内容|

|Database|UserDatabase||
|:-|:-|:-------------------:|:-|
|**Datatable**|**List of Fields**|**Description**|
|User_Table|CHAR(size) Username<br>CHAR(size) UserID<br>CHAR(size) Email<br>CHAR(size) Password|(1)用户名<br>(2)用户ID<br>(3)用户邮箱<br>(4)用户密码|
